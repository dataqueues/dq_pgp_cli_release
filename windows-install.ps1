# PowerShell Installation Script for DQ PGP CLI
...
Write-Host "DQ PGP CLI Installer" -ForegroundColor Cyan
...
function Get-LatestRelease {
    ...
    try {
        ...
        Write-Host "Ensure https://github.com/$repo exists and has releases" -ForegroundColor Blue
        throw
    }
}
...
try {
    ...
    if ((Test-Path "$InstallPath\dq-pgp.exe") -and (-not $Force)) {
        Write-Host "DQ PGP CLI is already installed at: $InstallPath" -ForegroundColor Yellow
    ...
    $windowsAsset = $release.assets | Where-Object {
        $_.name -like "*windows*amd64.exe" -or $_.name -eq "dq-pgp-windows-amd64.exe"
    }
    ...
    if (-not $windowsAsset) {
        ...
        Write-Host "Looking for: dq-pgp-windows-amd64.exe" -ForegroundColor Blue
        exit 1
    }
    ...
    $exePath = "$InstallPath\dq-pgp.exe"
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "Installation test completed with exit code: $LASTEXITCODE" -ForegroundColor Yellow
        }
    }
    catch {
    ...
    Write-Host "  3. Run: dq-pgp encrypted.txt" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Usage examples:" -ForegroundColor Cyan
    Write-Host "  dq-pgp encrypted.txt" -ForegroundColor Gray
    Write-Host "  dq-pgp encrypted.txt -k mykey.asc -v" -ForegroundColor Gray
    Write-Host "  dq-pgp -f encrypted.txt -k mykey.asc -p mysecret" -ForegroundColor Gray
    Write-Host ""
    Write-Host "For help: dq-pgp -h" -ForegroundColor Blue
    ...
    Write-Host "2. Download: dq-pgp-windows-amd64.exe" -ForegroundColor Gray
    Write-Host "3. Rename to: dq-pgp.exe" -ForegroundColor Gray
